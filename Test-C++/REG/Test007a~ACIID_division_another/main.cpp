...
if (dqC>dqCM){
    ...
    dqCMc=dqCMcase.str();
            ...
            if (dq==0){break;}
            if (r){dq--;}
//「//      for (unsigned i (((bsize-1)&Msz)+size); i>=size; i--){
//屈//        if (b[(i-bp)&Msz]){
//原//          unsigned temq ((a[(i+1-ap)&Msz]*10000+a[(i-ap)&Msz])/b[(i-bp)&Msz]);
//」//          if (temq&& temq<dq){dq=temq;}
//解//        }
//--//      }
/*漁*/      if (b[(size-bp)&Msz]){
/*夫*/       unsigned temq ((a[(size+1-ap)&Msz]*10000+a[(size-ap)&Msz])/b[(size-bp)&Msz]);
/*解*/       if (temq&& temq<dq){dq=temq;}
/*--*/      }
if (dq>dqM){dqM=dq;}
            for (unsigned i (size); i<bsize+1; i++){
